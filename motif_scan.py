...
def getoptions():
  ...
  parser.add_option('-d', type = 'string', dest = "pwm_dir", default = os.path.dirname(os.path.abspath(__file__)) + "/db/pwms", help = "Directory of PWMs [%default]")
  parser.add_option('-p', '--pseudocount', type = "float", dest = "pseudocount", default = 0, help = "Pseudocount for normalizing PWM. [%default]")
  parser.add_option('-r', '--rbpinfo', type = 'string', dest = 'rbpinfo', default = os.path.dirname(os.path.abspath(__file__)) + "/db/RBP_Information_all_motifs.txt", help = "RBP info for adding meta data to results. [%default]")
  parser.add_option('-t', '--type', type = 'string', dest = 'seqtype', default = "DNA", help = "Alphabet of input sequence (DNA or RNA). [%default]")
  parser.add_option('-m', '--minscore', type = 'float', dest = 'minscore', default = 6, help = "Minimum score for motif hits. [%default]")
  parser.add_option('-s', '--seq', type = 'string', dest = 'testseq', default = None, help = "Supply a test sequence to scan. FASTA files will be ignored.")
  parser.add_option('-c', type = "int", default = 1, dest = "cores", metavar = "CORES", help = "Number of processing cores [%default]")
  parser.add_option('-x', '--excel', action="store_true", dest="excel", default = False, help = "Format the RBP_ID column with =HYPERLINK(url) for " + "import into Excel [%default]")
  parser.add_option('-v', action="store_true", dest="version", default = False, help = "Print version number")
